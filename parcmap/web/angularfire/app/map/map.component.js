...
angular.
	...
	controller: function testController($scope,$firebaseObject,$firebaseArray,$rootScope,$timeout) {
		this.parcs=[];
        ...
        geocoder.geocode({'address': address}, function(results, status) {
  			...
  			var onKeyEnteredRegistration = this.geoQuery.on("key_entered", function(key, location, distance) {
  				...
  				if($rootScope.parcKeys.indexOf(key) === -1) {
	          		...
	          		positionRef.on("value", function(snapshot) {
	           			...
	           			console.log(parc.reviewsLength);
      	...
      	this.updateDistance = function(){
	        ...
	        for (var i=0; i<this.parcs.length; i++){
	      		...
	      		console.log(this.parcs[i].distance);
	      	}
	      	$timeout(function(){
        		$scope.$apply();
      		},100);
